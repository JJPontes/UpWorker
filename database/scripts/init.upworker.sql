-- Trigger para versionamento automático de GMUD ao atualizar chamado
CREATE OR REPLACE FUNCTION versionar_gmud()
RETURNS TRIGGER AS $$
BEGIN
...
-- View para facilitar consulta de GMUDs por chamado
CREATE OR REPLACE VIEW vw_gmuds_por_chamado AS
SELECT c.id AS chamado_id, c.uuid, g.versao, g.status, g.documento, g.criado_em, g.revisado_em, g.aprovado_em
FROM calleds c
JOIN gmuds g ON g.called_id = c.id;
